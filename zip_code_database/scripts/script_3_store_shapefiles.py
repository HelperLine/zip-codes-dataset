...
duplicate_shape_lists = {}


def recursive_list_size(nested_list):
    output = []

    for element in nested_list:
        if type(element) == list:
            output.append(recursive_list_size(element))
        else:
            if len(output) == 0:
                output.append(1)
            elif type(output[-1]) == int:
                output[-1] += 1
            else:
                output.append(1)
...
print(recursive_list_size([1, 2, [1, 2, [3]], 4]))
...
with open('../data/postleitzahlen-deutschland.json') as json_file:
    ...
    # print(json.dumps(zip_records[0], indent=4))
    ...
    for zip_record in tqdm(zip_records):
        try:
            ...
            shape_list = zip_record["fields"]["geo_shape"]["coordinates"]
            for shape in shape_list:
                for vertex in shape:
                    vertex[0] = round(vertex[0], 7)
                    vertex[1] = round(vertex[1], 7)

            if zip_code in duplication_zip_codes:
                if zip_code in duplicate_shape_lists:
                    duplicate_shape_lists[zip_code] += shape_list
                else:
                    duplicate_shape_lists[zip_code] = shape_list
                continue
            ...
            batch_to_be_added.append(mongodb_functions.format_shape_record_dict(zip_code, shape_list))

            if len(batch_to_be_added) == 100:
                mongodb_functions.mongodb_add_shape_record_dict_batch(batch_to_be_added)
                batch_to_be_added = []
...
for zip_code in tqdm(duplication_zip_codes):
    shape_list = duplicate_shape_lists[zip_code]
    batch_to_be_added.append(mongodb_functions.format_shape_record_dict(zip_code, shape_list))
...
"""
98711 has shape file but no in location -> no valid GCP geocode response -> deleted record manually
